...
def show_tables():
    format = request.args.get("format", "json")  # Get the format parameter, default to JSON
    data = data_fetch("show tables")
    if format == "xml":
        xml_data = xmltodict.unparse({"tables": {"table": data}})
...
def select_table(table):
    ...
    data = data_fetch(f"select * from {table}")
    if format == "xml":
        xml_data = xmltodict.unparse({"root": {"table": data}})
        return Response(xml_data, mimetype="application/xml")
    else:
...
def select_table_id(table, id):
    format = request.args.get("format", "json")
    data = data_fetch(f"select * from {table} where id='{id}'")
    if format == "xml":
        xml_data = xmltodict.unparse({"root": {"table": data}})
        return Response(xml_data, mimetype="application/xml")
    else:
